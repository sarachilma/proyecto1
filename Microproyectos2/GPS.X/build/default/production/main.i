        ...
        if(GPS_ReadSentence(buffer, sizeof(buffer), 1000)) {
            ...
            if(strstr(buffer, "$GPRMC")) {
                ...
                if(GPS_ParseGPRMC(buffer, lat, ns, lon, ew)) {
                    char formatted_lat[15] = {0};
                    char formatted_lon[15] = {0};


                    strncpy(formatted_lat, lat, 2);
                    strcat(formatted_lat, "\xDF");
                    strncat(formatted_lat, lat + 2, 5);
                    strcat(formatted_lat, "' ");
                    strcat(formatted_lat, ns);


                    strncpy(formatted_lon, lon, 3);
                    strcat(formatted_lon, "\xDF");
                    strncat(formatted_lon, lon + 3, 4);
